...
CONFIG = {
    ...
    # ---- V5: DUAL-MODE REGIME SWITCH ----
    "regime": {
        "adx_threshold_low": 20,         # ADX < this → mean-reversion mode
        "adx_threshold_high": 25,        # ADX > this → trend-following mode
        "adx_period": 14,                # ADX lookback
        "trend_ema_period": 50,          # Confirm trend direction
        # Mean-reversion mode params (inherits signal.rr_target, atr_sl_mult, max_hold_bars)
        "mr_rr_target": 2.0,             # TP = entry + rr_target * SL_distance
        "mr_atr_sl_mult": 1.25,          # SL distance multiplier
        "mr_max_hold_bars": 72,          # Max hold for mean-reversion trades
        # Trend-following mode params
        "tf_use_trailing_stop": True,    # Trailing stop instead of fixed TP
        "tf_trail_atr_mult": 2.5,        # Trail distance in ATR
        "tf_trail_activation_r": 1.0,    # Activate trail after 1R profit
        "tf_entry_atr_breakout": 0.3,    # Entry = breakout above/below recent high/low by N*ATR
        "tf_max_hold_bars": 120,         # Max hold for trend trades (20 days)
        "tf_pyramid_on_r": 1.0,          # Add 50% size at each +1R increment
    },
    # ---- END V4 ----
    "costs": {
...
def signal_generator(df, cfg):
    """V5: Dual-mode — mean-reversion (ADX < 20) or trend-following (ADX > 25)."""
    w = cfg["fusion"]; sigs = []
    reg = cfg.get("regime", {})
    adx_low = reg.get("adx_threshold_low", 20)
    adx_high = reg.get("adx_threshold_high", 25)
    
    for i in range(len(df)):
        r = df.iloc[i]
        if i < 200 or pd.isna(r.ema200): continue
        if r.chart_bias == "neutral" or pd.isna(r.atr): continue
        if pd.isna(r.adx): continue
        
        adx = r.adx
        if adx_low <= adx <= adx_high:
            continue  # Transition zone — no trade
        
        is_trending = adx > adx_high
        ...
        if is_trending:
            ...
            if (direction == "long" and not trend_up) or (direction == "short" and trend_up):
                continue  # Against trend — skip in TF mode
            
            # Entry: breakout above recent high/low by N*ATR
            breakout_dist = reg.get("tf_entry_atr_breakout", 0.3) * r.atr
            recent_high = df.high.iloc[max(0,i-5):i+1].max()
            recent_low = df.low.iloc[max(0,i-5):i+1].min()
            
            if direction == "long":
                entry = recent_high + breakout_dist
                # Check if current bar could fill at that level
                if r.high < entry: continue  # No fill this bar
            else:
                entry = recent_low - breakout_dist
                if r.low > entry: continue
            
            # SL: 2*ATR below/above entry
            sl_dist = 2.0 * r.atr
            sl = entry - sl_dist if direction == "long" else entry + sl_dist
            dist = sl_dist
            # TP: None (trailing stop handles exit) — signal sets a far TP for expiry only
            tp = entry + sl_dist * 5 if direction == "long" else entry - sl_dist * 5
            
            mode = "trend"
            regime_conv = 3 if trend_up == (direction == "long") else 2
        else:
            # === MEAN-REVERSION MODE ===
            entry = r.close  # Pullback entry
            atr_sl = reg.get("mr_atr_sl_mult", 1.25) * r.atr
            if direction == "long":
                sl = min(r.invalidation, entry - atr_sl)
                dist = entry - sl
                tp = entry + reg.get("mr_rr_target", 2.0) * dist
            else:
        ...
        if is_trending and not conflict: fused += 1.0  # Agreement bonus in trending
        conviction = int(min(5, max(1, round(fused))))
        
        rr = abs(tp - entry) / dist if dist > 0 else 999
        
        sigs.append(Signal(i, direction, entry, sl, tp, rr, conviction,
                           f"{mode} {direction} {r.chart_bias}/{r.whale_sent} ADX={adx:.0f}",
...
def run_backtest(df, cfg, test_start_idx=0):
    """V5: Dual-mode — different exit logic per mode (mean-reversion vs trend-following)."""
    rng = np.random.default_rng(cfg["seed"]+1)
    n = len(df); tf = cfg["timeframe_hours"]
    ...
    reg = cfg.get("regime", {})
    use_trail = reg.get("tf_use_trailing_stop", True)
    trail_mult = reg.get("tf_trail_atr_mult", 2.5)
    trail_act = reg.get("tf_trail_activation_r", 1.0)
    pyramid_r = reg.get("tf_pyramid_on_r", 1.0)
    ...
    for i in range(max(1,test_start_idx), n):
        ...
        if open_pos is not None:
            s,qty,notional,entry_px,bars_held,acc_funding,entry_idx,trail_stop,entry_mode,base_qty = open_pos
            ...
            if entry_mode == "trend":
                # === TREND-FOLLOWING EXIT: trailing stop only ===
                max_hold = reg.get("tf_max_hold_bars", 120)
                
                if use_trail and pd.notna(bar.atr):
                    trail_dist = trail_mult * bar.atr
                    if s.direction == "long":
                ...
                # Check hard SL first
                if s.direction=="long" and bar.low<=s.sl: hit,exit_px="SL",s.sl
                elif s.direction=="short" and bar.high>=s.sl: hit,exit_px="SL",s.sl
                
                # Trailing stop exit (after activation)
                if hit is None and use_trail and current_r >= trail_act:
                ...
                # Pyramid: add 50% at each +1R
                if hit is None and pyramid_r > 0 and current_r >= pyramid_r:
                    add_thresholds = [pyramid_r, pyramid_r*2, pyramid_r*3]
                    added_levels = int((qty - base_qty) / (base_qty * 0.5)) if base_qty > 0 else 0
                    for t in add_thresholds:
                        if current_r >= t and added_levels < len(add_thresholds):
                            if t >= pyramid_r * (added_levels + 1):
                                add_qty = base_qty * 0.5
                                add_notional = add_qty * bar.close
                                qty += add_qty
                                notional += add_notional
                                added_levels += 1
                
                # Expiry
                if hit is None and bars_held>=max_hold:
                    hit,exit_px="EXPIRY",bar.close
            else:
                # === MEAN-REVERSION EXIT: fixed TP/SL ===
                max_hold = reg.get("mr_max_hold_bars", 72)
            ...
            if hit:
                ...
                open_pos=(s,qty,notional,entry_px,bars_held,acc_funding,open_pos[7],trail_stop,entry_mode,base_qty)
        
        # New entry
        if open_pos is None and i>cooldown_until:
            sig=sig_by_idx.get(i-1)
            if sig is not None:
                # Determine mode from thesis string
                entry_mode = "trend" if sig.thesis.startswith("trend") else "mr"
                
                sized=risk_manager(sig,capital,cfg)  # V5 uses original risk_manager (no vol-adj)
                ...
                if sized.verdict=="approved" and dd_today>-cfg["limits"]["daily_loss_limit_pct"]:
                    fill=sig.entry  # Use signal's computed entry (pullback or breakout)
                    if entry_mode == "mr":
                        fill=bar.open*(1+slip_pct/100) if sig.direction=="long" else bar.open*(1-slip_pct/100)
                    qty=sized.qty; notional=qty*fill
                    init_trail = fill
                    base_qty = qty
                    open_pos=(sig,qty,notional,fill,0,0.0,i,init_trail,entry_mode,base_qty)
